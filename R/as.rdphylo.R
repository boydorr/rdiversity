...
#' \code{terminal.taxa} \tab - na data_frame containing components associated 
...
rdphylo <- function(pds.abundance, tree) {
  ...
  if(ncol(pds.abundance)==1) {
    tmp <- cbind(tip.nodes, pds.abundance, 1)
    colnames(tmp) <- c("tip.node", "pds.abundance", "subcommunity")
    terminal.taxa <- merge(terminal.taxa, tmp)
  }else {
    tmp <- lapply(as.list(1:ncol(pds.abundance)), function(x)
      cbind.data.frame(tip.nodes, pds.abundance[,x], colnames(pds.abundance)[x]))
    tmp <- do.call(rbind, tmp)
    colnames(tmp) <- c("tip.node", "pds.abundance", "subcommunity")
    terminal.taxa <- merge(terminal.taxa, tmp, all = TRUE)
  }
