...
test_that ("The qDZ() function works for a single population", {
  expect_equivalent(qDZ(set.collection(weights), 1:2), 
                    c(qD(weights, 1:2)))
  expect_equivalent(qDZ(set.collection(weights, zmatrix = Z1), 
                        qs = c(0:3,Inf)), 
                    rep(1,5))
})
...
expect_equivalent(qDZ(set.collection(manyweights, zmatrix = Z2), c(0:2, Inf)),
...
test_that ("Generate warnings, but normalise and calculate diversities", {
  # expect_equivalent(qDZ(set.collection(pop/sum(pop)), 1), 2)  
  expect_error(qDZ(set.collection(pop), 1))  
})
