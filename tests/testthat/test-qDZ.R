...
test_that ("The qDZ() function works for a single population", {
  expect_equivalent(qDZ(as.supercommunity(weights), 1:2), 
                    c(qD(weights, 1:2)))
  expect_equivalent(qDZ(as.supercommunity(weights, zmatrix = Z1), 
                        qs = c(0:3,Inf)), 
                    rep(1,5))
})
...
expect_equivalent(qDZ(as.supercommunity(manyweights, zmatrix = Z2), c(0:2, Inf)),
