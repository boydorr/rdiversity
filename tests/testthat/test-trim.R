...
test_that("Setting interval to < 1 returns correct results", {
  ...
  ps_ans = cbind(A = c(0,0,0,0,0,0,0,0.6,0.1,0,0.4,0)/3.7,
...
test_that("Metacommunity G is larger answer for ultrametric trees", {
  # Non-ultrametric tree
  Ntips <- 30
  n_tree <- ape::rtree(Ntips)
  n_tree$tip.label <- paste0("sp", 1:Ntips)
  # Partition
  partition <- matrix(sample(Ntips,Ntips), ncol=1)
  row.names(partition) <- n_tree$tip.label
  partition <- partition / sum(partition)
  # Extend tips to make ultrametric tree
  u_tree <- ape::compute.brlen(n_tree, method = "Grafen")
  # Generate metacommunitites
  u_meta <- metacommunity(partition, u_tree)
  n_meta <- metacommunity(partition, n_tree)
  # Calculate diversity
  u_div <- meta_gamma(u_meta,0)$diversity
  n_div <- meta_gamma(n_meta,0)$diversity
  # Test that n_div < u_div
  # testthat::expect_lt(n_div, u_div)
})
