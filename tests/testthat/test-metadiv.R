...
test_that("metacommunity diversity across multiple populations", {
  ...
  expect_equivalent(as.matrix(metadiv(normalised_alpha(metacommunity(pop)), 0)$diversity), t(7/4))
  expect_equivalent(as.matrix(metadiv(raw_beta(metacommunity(pop)), 0)$diversity), t(19/48))
  expect_equivalent(as.matrix(metadiv(normalised_beta(metacommunity(pop)), 0)$diversity), t(13/12))
  expect_equivalent(as.matrix(metadiv(raw_rho(metacommunity(pop)), 0)$diversity), t(11/4))
  expect_equivalent(as.matrix(metadiv(normalised_rho(metacommunity(pop)), 0)$diversity), t(15/16))
