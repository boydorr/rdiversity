...
test_that("subdiv() returns expected values", {
  ...
  expect_equal(
    raw_alpha@subcommunity_weights,
    setNames(c(0.25, 0.25, 0.5), c(letters[1:3]))
  )
  expect_equal(
    raw_alpha@type_weights,
    as.matrix(data.frame(
      a = c(0.5, 0.5, 0),
      b = c(1, 0, 0),
      c = c(0.75, 0.25, 0)
  ...
  expect_equal(raw_alpha@similarity_parameters, list(
    transform = NA,
    k = NA,
    normalise = NA,
    max_d = NA
  ...
  # Results
  expect_equivalent(
    subdiv(norm_alpha(meta), 0)$diversity,
    c(2, 1, 2)
  )
  expect_equivalent(
    subdiv(raw_beta(meta), 0)$diversity,
    c(1 / 4, 1 / 3, 1 / 2)
  )
  expect_equivalent(
    subdiv(norm_beta(meta), 0)$diversity,
    c(1, 4 / 3, 1)
  )
  expect_equivalent(
    subdiv(raw_rho(meta), 0)$diversity,
    c(4, 3, 2)
  )
  expect_equivalent(
    subdiv(norm_rho(meta), 0)$diversity,
    c(1, 3 / 4, 1)
  )
  expect_equivalent(
    subdiv(raw_gamma(meta), 0)$diversity,
    c(8 / 3, 4 / 3, 2)
  )
})
